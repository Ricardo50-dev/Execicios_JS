...
const criarItem = (tarefa, status, indice) => {
    ...
    item.innerHTML = `
        <input type="checkbox" class="cb" ${status} data-indice=${indice}>
        <div class="desc-tarefa">${tarefa}</div>
        <button type="button" class="btn-close" data-indice=${indice}>✖</button>
    `
    // definir o novo item como filho do nosso todoList (div)    
    document.getElementById('campo-lista').appendChild(item)
}
...
const limparTarefas = () => {
    ...
    const todoList = document.getElementById('campo-lista')
...
document.getElementById('input-tarefa').addEventListener('keypress', inserirItem)
document.getElementById('campo-lista').addEventListener('click', clickItem)
